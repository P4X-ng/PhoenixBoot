...
menuentry "KVM Snapshot Jump" {
    ...
    linuxefi ($esp)/EFI/PhoenixGuard/vmlinuz root=UUID=${ROOT_UUID} ro quiet ${IOMMU_PARAMS} systemd.unit=kvm-snapshot-jump.service
    initrdefi ($esp)/EFI/PhoenixGuard/initrd.img
}
GRUB
...
menuentry "Remediation Boot (fwupd/vendor)" {
    ...
    insmod fat
    search --no-floppy --file --set=esp /EFI/PhoenixGuard/vmlinuz
    linuxefi ($esp)/EFI/PhoenixGuard/vmlinuz root=UUID=${ROOT_UUID} ro quiet systemd.unit=pg-remediate.service
    initrdefi ($esp)/EFI/PhoenixGuard/initrd.img
}
GRUB
